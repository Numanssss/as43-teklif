...
logo_b64 = open('logo_base64.txt').read().strip()

code_template = '''# --- 0.DEV0 GEÇERSİZ SÜRÜM HATASINI ÇÖZEN GLOBAL MONKEYPATCH ---
try:
...
# Logo dosyasini otomatik olustur (Streamlit Cloud ve Codespaces uyumlulugu icin)
if not os.path.exists("logo.png"):
    try:
        with open("logo.png", "wb") as f:
            f.write(base64.b64decode("__LOGO_BASE64__"))
    except Exception:
        pass
...
def get_transparent_logo_base64():
    ...
    try:
        ...
        datas = img.getdata()
        new_data = []
        # Beyaz ve yakin tonlarini seffafa cevir
        for item in datas:
...
st.markdown("""
    <style>
    /* Global sayfa arka planı ve Android/Mobil Kaydirma Duzeltmesi */
...
final_code = code_template.replace('__LOGO_BASE64__', logo_b64)
open('app.py', 'w').write(final_code)
